...
@pytest.mark.asyncio
async def test_platform_map_exposes_nexusops_domain_boundaries(test_client, monkeypatch):
    ...
    body = response.json()
    assert body["umbrella"] == "NEXUSBIGDATA"
    assert body["product"] == "NexusOps AI"
    assert body["current_core"] == "ZENTHRA.CORE_SECURITY"
    ...
    assert domains["nexusflow"]["contracts"] == ["WorkflowEngine"]
    assert "app.ares" in domains["nexusflow"]["current_backend_mapping"]
    ...
    assert domains["nexusvault"]["contracts"] == ["KnowledgeRetriever"]
    assert "app.intelligence" in domains["nexusvault"]["current_backend_mapping"]
    assert domains["nexusapi"]["contracts"] == ["ToolRegistry"]
    assert "app.secops" in domains["nexusapi"]["current_backend_mapping"]
...
async def test_platform_readiness_exposes_domain_statuses(test_client, monkeypatch):
    ...
    assert domains["blacknode"]["evidence"]["gateway"]["contract"] == (
        "nexusops.blacknode.security_gateway.v1"
    )
    ...
    assert domains["nexusvault"]["status"] == "ready"
    assert domains["nexusvault"]["evidence"]["retriever"]["contract"] == (
        "nexusops.nexusvault.knowledge_retriever.v1"
    )
    assert domains["nexusvault"]["evidence"]["provider"]
    assert "domains" in domains["nexusvault"]["evidence"]

    assert domains["nexusapi"]["status"] == "ready"
    assert domains["nexusapi"]["evidence"]["registry"]["contract"] == (
        "nexusops.nexusapi.tool_registry.v1"
    )
    assert domains["nexusapi"]["evidence"]["registered_tools"]
    assert domains["nexusapi"]["evidence"]["action_tools"]
    provider_names = {
        item["provider"] for item in domains["nexusapi"]["evidence"]["devsecops_providers"]
    }
    ...
    nexusflow_checks = {item["name"]: item for item in domains["nexusflow"]["checks"]}
    assert nexusflow_checks["workflow_engine_contract"]["passed"] is True
...
def test_nexusapi_registry_classifies_action_tools():
    status = nexusapi_registry.build_status()
    actions = {item["name"]: item for item in status["action_tools"]}

    assert status["contract"] == "nexusops.nexusapi.tool_registry.v1"
