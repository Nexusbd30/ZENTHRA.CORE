# VAELQORIX AI Platform Architecture

Status: Development Folder Architecture v2.0

## Architectural Decision

VAELQORIX AI will evolve from the current VAELQORIX.XDR_COMMAND modular backend into a domain-oriented enterprise platform.
...
| VaelqorixFlow | Workflows and orchestration | `app.ares`, `app.services.autonomy_service` |
| BlackNode | Security and governance | `app.core`, `app.middlewares`, `app.models.audit_record`, `app.ares.approval` |
| VaelqorixVault | Knowledge and RAG | `app.intelligence`, `app.db.vector`, `app.models.knowledge_document` |
| VaelqorixAPI | Tools and integrations | `app.actions`, `app.identity`, `app.secops`, `app.ingestion` |
